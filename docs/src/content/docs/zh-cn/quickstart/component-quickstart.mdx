---
title: Component 快速入门
description: 使用 ComponentsCardModel 和 CardComponent 组织可复用卡牌行为。
sidebar:
order: 3
---
...
<Aside type="note" title="先完成环境配置">
    如果你的项目还没有引用 MinionLib，请先阅读[环境配置](/MinionLib/zh-cn/quickstart/environment/)。Component 依赖 NuGet
    包中的 Source Generator，不建议用本地 DLL 引用。
</Aside>
...
示例项目中的 `MinionLib.Example/Components/` 也有可直接对照的组件形态：普通出牌组件、费用改写组件、右键组件、多选项组件和跨时机组件都能在这里找到。
